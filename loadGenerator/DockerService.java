...
public class DockerService {
	 ...
	 * @return
	 */
	public Map<String,ContainerResourceUsageBean> getContainerResourceUsage(){
		Map<String,ContainerResourceUsageBean> map = new HashMap<String,ContainerResourceUsageBean>();
		try {  
			String[] cmd = {"/bin/sh","-c","docker stats --no-stream --format '{{.Name}}:{{.CPUPerc}}:{{.MemPerc}}'"}; 
			String line = null,err;
			Process process = Runtime.getRuntime().exec(cmd); 
			BufferedReader br = new BufferedReader(new InputStreamReader(process.getErrorStream()));
			InputStreamReader isr = new InputStreamReader(process.getInputStream());
	 ...
	 * @return 操作结果
	 */
	public int execCommand(String containerName,String command){

		String[] cmd = {"/bin/sh","-c","docker exec -d "+containerName+" "+command}; 
		//System.out.println("docker exec -d "+containerName+" "+command);
		String err;
		try {
			Process process = Runtime.getRuntime().exec(cmd); 
			BufferedReader br = new BufferedReader(new InputStreamReader(process.getErrorStream()));
	...
	public int getTaskInstanceCount(String command){ 
		...
		try {  
			...
			while (((err = br.readLine()) != null||(line = input.readLine()) != null)) {
				if(err==null){   
					result=Integer.parseInt(line.trim())-2;
				}else{
					System.out.println(err);
				}
			}   
	 ...
	 * @return 正在运行的数量
	 */
	public int getTaskProduceCount(String containerName,String command){ 
		int result=0; 
		try {  
			String[] cmd = {"/bin/sh","-c","docker exec -i "+containerName+" "+command}; 
			String line = null,err;
			Process process = Runtime.getRuntime().exec(cmd); 
			BufferedReader br = new BufferedReader(new InputStreamReader(process.getErrorStream()));
			InputStreamReader isr = new InputStreamReader(process.getInputStream());
